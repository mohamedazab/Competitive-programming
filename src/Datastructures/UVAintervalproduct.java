...
public class UVAintervalproduct {
...
int N = 1; while(N < n) N <<= 1; //padding
		...
		for (int i = 0; i < q; i++) {
			 fs=sc.nextLine().split(" ");
			String  op = fs[0];
			int val=Integer.parseInt(fs[1]);
			int rep=Integer.parseInt(fs[2]);
			if(op.equals("P")){
				int x=sm.query(val, rep);
				if(x==0)
					out.print("0");
				else
					if(x<0)
	...
	static class SegmentTree {		// 1-based DS, OOP
		...
		void build(int node, int b, int e)	// O(n)
		{
			...
			else						
			{
				...
				if(sTree[node<<1]*sTree[(node<<1)+1]<0)
					sTree[node] = -1;
					else
						if(sTree[node<<1]*sTree[(node<<1)+1]>0)
							sTree[node] = 1;
